...
const port = 8000;
const { Functions } = require("faunadb");
message.use(
	...
	async function getuser(ref) // req.body.ref
		{
			return serverClient.query(
				q.Get(q.Ref(q.Collection('ChatRooms'), ref))
				)
		}
...
message.post('/changeChatName', function(req, res){ 
		...
		if (true)
		{
			async function updater()
			{
				try
				{
					...
					serverClient.query(
							q.Update(
								q.Ref(q.Collection("ChatRooms"), req.body.ref),
								{
									data: {
										name : newName
...
message.post('/changeChatImage', function(req, res){ 
...
message.post("/getMessages", function(req, res) // send in the ref of the chatroom
{
...
message.post("/createChatRoom", function(req, res)
{
	...
	async function createRoom()
	{
		...
		if (true)
		{
			serverClient
				.query(
					q.Create(q.Collection("ChatRooms"),
					{
						data: req.body,
					})
				)
...
message.post("/deleteChatRoom", function(req, res)
{
	async function ok()
	{
		authStatus = await testAuth(req.headers["authorization"]);
...
message.post("/addMember", function(req, res){// adds a member to the chatroom. Specifically the ref of that member will be stored. Send the ref as an array of one term
	...
	message.post('/removeMember', function(req, res){
